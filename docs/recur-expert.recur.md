...
persona = Recur expert in query-first hierarchy, trace-id, eventness and bounded Warp work.
agent = inspect and explain first; implement only when the user requests it.
pull.first = read julia-expert/references/recur-playbook.md Warp specialization; recur warp
pull.then = recur warp slices main.command.prompt.discovery; read docs/main.command.prompt.discovery.readme.md and its warp-map.json
...
checkpoint = df946cf, 2026-09-06; re-query current state before resuming.
do.not.disturb = No automatic command execution, Git operations, deletion, semantic merge/split or invented acceptance.
ready.state = Identity-policy and prompt.discovery are complete; reveal.persona-skills remains planned. Use live maps and recorded test configurations as the scope contract.
